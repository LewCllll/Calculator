...
class ViewController: UIViewController {
    ...
    @IBAction func Negative(_ sender: UIButton) {
        if Double(self.displayLabel.text!)! > 0 {
            self.displayLabel.text = "-\(self.displayLabel.text!)"
        } else if Double(self.displayLabel.text!)! < 0 {
            self.displayLabel.text = self.displayLabel.text?.replacingOccurrences(of: "-", with: "")
        }
    }
    ...
    @IBAction func Percentage(_ sender: UIButton) {
        ...
        self.displayLabel.text = "\(Double(self.displayLabel.text!)! * 0.01)"
        //Prevent displaying numbers with a ".0" suffix
        if (self.displayLabel.text?.hasSuffix(".0"))!{
        self.displayLabel.text = self.displayLabel.text?.replacingOccurrences(of: ".0", with: "")
        }
    }
